...
local build_grammar = wrap_env(debug_grammar, function(root)
  ...
  local g = P({
    ...
    Local = key("lokal") * ((op("*") + op("^")) / mark("declare_glob") + Ct(NameList) / mark("declare_with_shadows")),
    Import = key("import") * Ct(ImportNameList) * SpaceBreak ^ 0 * key("from") * Exp / mark("importer"),
    ...
    Return = key("returner") * (ExpListLow / mark("explist") + C("")) / mark("returner"),
    WithExp = Ct(ExpList) * Assign ^ -1 / format_assign,
    With = key("med") * DisableDo * ensure(WithExp, PopDo) * key("gør") ^ -1 * Body / mark("med"),
    Switch = key("scenarie") * DisableDo * ensure(Exp, PopDo) * key("gør") ^ -1 * Space ^ -1 * Break * SwitchBlock / mark("scenarie"),
    SwitchBlock = EmptyLine ^ 0 * Advance * Ct(SwitchCase * (Break ^ 1 * SwitchCase) ^ 0 * (Break ^ 1 * SwitchElse) ^ -1) * PopIndent,
    SwitchCase = key("når") * Ct(ExpList) * key("så") ^ -1 * Body / mark("hændelse"),
    SwitchElse = key("ellers") * Body / mark("ellers"),
    IfCond = Exp * Assign ^ -1 / format_single_assign,
    IfElse = (Break * EmptyLine ^ 0 * CheckIndent) ^ -1 * key("ellers") * Body / mark("ellers"),
    IfElseIf = (Break * EmptyLine ^ 0 * CheckIndent) ^ -1 * key("ellershvis") * pos(IfCond) * key("så") ^ -1 * Body / mark("ellershvis"),
    If = key("hvis") * IfCond * key("så") ^ -1 * Body * IfElseIf ^ 0 * IfElse ^ -1 / mark("hvis"),
    Unless = key("unless") * IfCond * key("then") ^ -1 * Body * IfElseIf ^ 0 * IfElse ^ -1 / mark("unless"),
    While = key("sålænge") * DisableDo * ensure(Exp, PopDo) * key("gør") ^ -1 * Body / mark("sålænge"),
    For = key("for") * DisableDo * ensure(Name * sym("=") * Ct(Exp * sym(",") * Exp * (sym(",") * Exp) ^ -1), PopDo) * key("gør") ^ -1 * Body / mark("for"),
    ForEach = key("for") * Ct(AssignableNameList) * key("i") * DisableDo * ensure(Ct(sym("*") * Exp / mark("unpack") + ExpList), PopDo) * key("gør") ^ -1 * Body / mark("forhver"),
    Do = key("gør") * Body / mark("gør"),
    ...
    CompForEach = key("for") * Ct(AssignableNameList) * key("i") * (sym("*") * Exp / mark("unpack") + Exp) / mark("forhver"),
    ...
    Update = ((sym("..=") + sym("+=") + sym("-=") + sym("*=") + sym("/=") + sym("%=") + sym("eller=") + sym("og=") + sym("&=") + sym("|=") + sym(">>=") + sym("<<=")) / trim) * Exp / mark("update"),
    ...
    SimpleValue = If + Unless + Switch + With + ClassDecl + ForEach + For + While + Cmt(Do, check_do) + sym("-") * -SomeSpace * Exp / mark("minus") + sym("#") * Exp / mark("length") + sym("~") * Exp / mark("bitnot") + key("ikke") * Exp / mark("ikke") + TblComprehension + TableLit + Comprehension + FunLit + Num,
